...
${ctx}`;
  ...
  if (skills && skills.length > 0) {
    skillsContext = `\n\n## Available Skills\n\n${formatSkillsForPrompt(skills)}\n\n**How to use skills — MANDATORY workflow:**\n1. **BEFORE** using \`lark-cli\` or any skill-specific tool, you **MUST** call \`Skill(skill_name="...")\` first\n2. The Skill tool returns the complete SKILL.md with exact command syntax, parameters, and examples\n3. **NEVER guess lark-cli command names** — always read the SKILL.md guide first\n4. Example: \`Skill(skill_name="lark-im")\` → read the returned commands → then call \`shell\` with the correct command\n\n⚠️ Many lark-cli shortcuts look similar but are different commands. Using a wrong command (e.g. \`+chat-messages-search\` instead of \`+messages-search\`) will fail silently. Always verify against SKILL.md.\n\n**Skill Installation Rules — CRITICAL:**\n- When a user asks to "install a skill" or "learn a skill", you **MUST** save the skill files to \`.fan_bot/skills/<skill-name>/\` directory\n- The only valid skill installation directory is \`.fan_bot/skills/\` (relative to the project root)\n- **NEVER** use external CLI tools (e.g. \`@byted/aipaas\`, \`@tiktok-fe/skills\`, \`npx\` packages) to install skills\n- **NEVER** install skills to \`~/.openclaw/skills/\` or any directory outside \`.fan_bot/skills/\`\n- To install a skill: create the directory \`.fan_bot/skills/<name>/\` and write the \`SKILL.md\` (plus any scripts/references) into it\n- After installation, call \`Skill(action="confirm", skill_name="<name>")\` if the skill was auto-extracted, or simply inform the user it is ready`;
  }
