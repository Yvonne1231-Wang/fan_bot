...
const log = createDebug('feishu:card');
...
export interface FeishuCardClient {
  ...
  patchInteractiveCard(messageId: string, cardJson: string): Promise<boolean>;
}
...
export class StreamingCardRenderer {
  ...
  async onThinking(thinkingText: string): Promise<void> {
    ...
    const step = this.state.steps.find(
      (s) => s.id === this.currentThinkingStepId,
    );
    if (step) {
  ...
  async onToolStart(
    ...
    if (this.currentThinkingStepId) {
      const thinkingStep = this.state.steps.find(
        (s) => s.id === this.currentThinkingStepId,
      );
      if (thinkingStep) {
    ...
    if (parentToolUseId) {
      ...
      log.warn(
        `parentToolUseId ${parentToolUseId} not found, adding to top level`,
      );
    }
  ...
  async onToolEnd(
    ...
    if (parentToolUseId) {
      const parentStep = this.toolUseIdToStep.get(parentToolUseId);
      if (parentStep && parentStep.childSteps) {
        const found = this.findAndMarkStep(
    ...
    const step = [...this.state.steps]
      ...
      .find(
        (s) =>
          s.type === 'tool' &&
          (s.label === toolName || s.label === displayName) &&
          s.status === 'running',
      );
    if (step) {
  ...
  private findAndMarkStep(
    ...
    for (const s of steps) {
      if (
        s.childSteps &&
        this.findAndMarkStep(s.childSteps, toolName, displayName, status)
      ) {
        return true;
      }
    }
  ...
  private buildCard(): object {
    ...
    if (this.state.phase === 'error' && this.state.errorMessage) {
      elements.push({
        tag: 'markdown',
        content: `❌ **错误**\n\n${this.truncate(this.state.errorMessage, 500)}`,
        text_size: 'normal',
      });
    }
  ...
  private buildCardHeader(): object {
    ...
    > = {
      ...
      generating: { template: 'turquoise', icon: '✍️', text: '生成回答中...' },
      completed: { template: 'green', icon: '✅', text: '已完成' },
      error: { template: 'red', icon: '❌', text: '出错了' },
      aborted: { template: 'grey', icon: '⏸️', text: '已中断' },
    };
    ...
    try {
      ...
      if (byteSize > StreamingCardRenderer.CARD_BYTE_LIMIT) {
        ...
        if (this.state.liveThinkingText.length > 500) {
          this.state.liveThinkingText =
            this.state.liveThinkingText.slice(0, 500) + '...';
        ...
        if (byteSize > StreamingCardRenderer.CARD_BYTE_LIMIT) {
          ...
          while (lo < hi && iterations < maxIterations) {
            ...
            if (
              this.getByteLength(testJson) <=
              StreamingCardRenderer.CARD_BYTE_LIMIT
            ) {
              lo = mid;
            } else {
              hi = mid - 1;
