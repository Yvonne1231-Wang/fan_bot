...
- **用户隔离** - 记忆按用户隔离，互不干扰
- **上下文压缩** - 上下文过长时自动压缩
- **艾宾浩斯曲线** - 模仿人脑遗忘规律，被访问的记忆强度恢复，衰减重置
...
模仿人脑遗忘规律，实现记忆强度的动态管理：

- **记忆强度 (memoryStrength)** - 每条记忆有 0.0-1.0 的强度值
- **指数衰减** - 强度随时间按半衰期指数衰减
- **复习恢复** - 被访问时强度恢复，越弱的记忆恢复越多（`recoveryBoost = 0.3 × (1 - currentStrength)`）
- **衰减重置** - 访问后从 `lastAccessedAt` 重新开始衰减，而非从创建时间
- **半衰期延长** - 频繁访问的记忆半衰期延长，最多延长到基础的 3 倍（14天 → 42天）
- **强化消退** - 30 天不访问，强化效果按半衰期消退
